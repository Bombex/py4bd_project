...
def get_similar_images(uploaded_file, num_similar_images):
    with ((st.spinner('Searching similar images... (may take up to a minute)'))):
        ...
        result = client.query.get(WEAVIATE_CLASS, WEAVIATE_FIELDS).with_near_vector(
            nearVector
        ).with_limit(num_similar_images).with_additional(['certainty']).do()
...
def display_results(col1, col2, col3, loaded_data, num_similar_images):
    ...
    col3.subheader('ㅤ')
    for i, data in enumerate(loaded_data['data']['Get'][WEAVIATE_CLASS][:num_similar_images]):
        ...
        col.image(image, caption=label + ', similarity: ' + percentage_string, use_column_width=True)
